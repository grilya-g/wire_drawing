...
reds_ = [1.5, 2.5, 5., 10., 20.]
reductions = [17., 19.]
# 
cals_ = [0., 0.1, 0.3, 0.5, 0.75, 1.]
    ...
    for cp in cal_parts:
        ...
        for half_angle in half_angles:
            for vel in velocities:
                ...
                label = 'zeides_2a_' + str(full_angle) + '_red_' + str(red_100)+ '_cal_' + str(cp_100) + '_vel_' + str(vel)
                label = str(label)
                ...
                s.Line(point1=(r_1, offset-cp*d_0), point2=(r_1-r_1*math.sin(math.radians(180+half_angle)), offset-cp*d_0 -d_0*math.sin(math.radians(180-half_angle)) ))
                ...
                s.Line(point1=(r_1, offset+0.0), point2=(r_1+2.5*d_0*math.sin(math.radians(half_angle)), offset+2.5*d_0*math.cos(math.radians(half_angle))))



                
                p = mdb.models[label].Part(name=current_die_name, dimensionality=AXISYMMETRIC, type=ANALYTIC_RIGID_SURFACE)
                ...
                if cp_len == 0:
                    ...
                    mdb.models[label].parts[current_die_name].ReferencePoint(
                        point=
                        mdb.models[label].parts[current_die_name].vertices[3])
                ...
                mdb.models[label].interactions['Int-1'].setValues(
                    main=region1, mechanicalConstraint=KINEMATIC, sliding=FINITE, 
                ...
                mdb.models[label].interactions['Int-2'].setValues(
                    ...
                    mdb.models[label].rootAssembly.instances[current_die_instance_name].surfaces['For_contact']
                ...
                steptime = round(elong_coef * ((abs(offset) + len_base + cp*d_0 + fora)) / velo_v2, 5)   
